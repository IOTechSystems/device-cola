...
package org.edgexfoundry.pkg.data;
...
import org.edgexfoundry.pkg.handler.CoreDataMessageHandler;
...
public class ObjectStore {
  ...
  public void put(Device device, ResourceOperation operation, String value) {
    ...
    List<COLAObject> objectsList = createObjectsList(operation, device);
    String deviceId = device.getId();
    List<Reading> readings = new ArrayList<>();

    for (COLAObject obj: objectsList) {
  ...
  private List<COLAObject>
      createObjectsList(ResourceOperation operation, Device device) {
    Map<String, COLAObject> objects = profiles.getObjects().get(device.getName());
    List<COLAObject> objectsList = new ArrayList<COLAObject>();

    if (operation != null && objects != null) {
      COLAObject object = objects.get(operation.getObject());
  ...
  private String transformResult(String result, COLAObject object,
