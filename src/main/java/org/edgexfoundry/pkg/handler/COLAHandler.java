...
package org.edgexfoundry.pkg.handler;
...
import org.edgexfoundry.pkg.domain.ResponseObject;
import org.edgexfoundry.pkg.domain.ScanList;
import org.edgexfoundry.pkg.domain.Transaction;
...
import org.edgexfoundry.pkg.cola.DeviceDiscovery;
import org.edgexfoundry.pkg.cola.ObjectTransform;
import org.edgexfoundry.pkg.cola.COLADriver;
...
@Service
public class COLAHandler {

  private static final EdgeXLogger logger =
      EdgeXLoggerFactory.getEdgeXLogger(COLAHandler.class);

  @Autowired
  private COLADriver driver;
  ...
  @Value("${cola.device.init:#{null}}")
  private String colaInit;
  @Value("${cola.device.init.args:#{null}}")
  private String colaInitArgs;
  @Value("${cola.device.remove:#{null}}")
  private String colaRemove;
  @Value("${cola.device.remove.args:#{null}}")
  private String colaRemoveArgs;
  ...
  public void initializeDevice(Device device) {
    if (colaInit != null && commandExists(device, colaInit)) {
      executeCommand(device, colaInit, colaInitArgs);
    }
  ...
  public void disconnectDevice(Device device) {
    if (colaRemove != null && commandExists(device, colaRemove)) {
      executeCommand(device, colaRemove, colaRemoveArgs);
    }
  ...
  private void executeOperations(Device device, String commandName, String arguments,
    ...
    Map<String, COLAObject> objects = profiles.getObjects().get(deviceName);
    ...
    for (ResourceOperation operation: operations) {
      ...
      COLAObject object =
          getCOLAObject(objects, objectName, transactionId);
  ...
  private COLAObject getCOLAObject(Map<String,
      COLAObject> objects, String objectName, String transactionId) {
    COLAObject object = objects.get(objectName);
